...
|---------|---------------|---------------|
| **systemd** | `birthDate` field in userdb user records, `--birth-date` flag in homectl | Merged to main, not yet in a stable release |
| **xdg-desktop-portal** | ParentalControls portal with `QueryAgeBracket` D-Bus method | Draft PR #1922, not yet merged |
| **accountsservice** | Birth date storage in user account records | MR #176, in progress |
| **archinstall** | Required birth date field during user creation | PR #4290, open |
